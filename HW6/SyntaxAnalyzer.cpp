...
identifierBaseType SyntaxAnalyzer::factor()
{
    ...
    else if (word.symbol == "IDENFR")
    {
        ...
        if (word.symbol == "LBRACK")
        {
            ...
            Symbol newSymbol1(newSymbolName(), VAR, INT);
            Symbol m1Symbol("18373584CONSTINT", 1);
            middleCodeManager.addMiddleCode(OpMinus, dim1Symbol, m1Symbol, newSymbol1);
            Symbol arrayDim2(newSymbolName(), symbolTableManager.findSymbol(idName).dim2);
            Symbol newSymbol2(newSymbolName(), VAR, INT);
            middleCodeManager.addMiddleCode(OpMulti, newSymbol1, arrayDim2, newSymbol2);
